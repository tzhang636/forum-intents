...
public class UnigramFeatures extends NGramFeatures {
	...
	public Map<Integer, Double> getWeights(ForumThread thread, String jsonDir) throws Exception {
		...
		if (sents.isEmpty()) {
			sents = Processor.getSents(post);
		}
		List<List<String>> toks = Processor.tokenize(sents);
		List<List<String>> ltoks = Processor.listToLowerCase(toks);
		
		for (List<String> ltokList : ltoks) {
			for (String ltok :ltokList) {
				if (!tok2id.containsKey(ltok)) {
					continue;
				}
