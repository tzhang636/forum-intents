...
public class Liwc {
	...
	public static void createInput(List<ForumThread> threads) throws Exception {
		for (ForumThread thread : threads) {
			...
			String firstPost = thread.getFirstPost();
			List<String> qSents = Processor.getQSents(firstPost);
			for (String qSent : qSents) {
				pw.print(qSent + " ");
			}
			pw.close();
		}
	...
	public static Map<String, Map<String, Double>> parseOutput() throws Exception {
		BufferedReader br = new BufferedReader(new FileReader("data/liwc/output/LIWC results.dat"));
